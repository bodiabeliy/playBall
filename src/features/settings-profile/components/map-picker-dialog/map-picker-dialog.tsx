...
export function MapPickerDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      setPosition(
        initialPosition && isFinite(initialPosition.lat) && isFinite(initialPosition.lng) ? initialPosition : fallback
      )
    }
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg" PaperProps={{ sx: { borderRadius: '12px' } }}>
